...
export default function PetroFlow() {
  const product = useAsync(() => fetchProductBySlug('petroflow'), []);
  ...
  if (product.error || !product.data) {
    return (
      <div className="mx-auto max-w-2xl px-4 py-24 text-center">
        ...
        <p className="mt-3 text-[var(--text-muted)]">This product page isn't available right now.</p>
    ...
    <>
        ...
        visual={
          <div className="relative h-72 w-full overflow-hidden rounded-3xl border border-[var(--border)] bg-[var(--surface)]">
            {p.image ? (
              ...
              <div className="flex h-full w-full items-center justify-center text-7xl text-[#00AFFF]">
                {p.icon ? p.icon : '⛽'}
              </div>
            )}
          </div>
      ...
      <Section>
        <div className="grid gap-10 lg:grid-cols-[1.5fr_1fr]">
          <div>
            <h2 className="text-2xl font-bold text-[var(--text-primary)]">About {p.name}</h2>
            <p className="mt-4 text-base leading-relaxed text-[var(--text-secondary)]">{p.description}</p>

            {features.length > 0 ? (
              <>
                <h2 className="mt-10 text-2xl font-bold text-[var(--text-primary)]">Key features</h2>
