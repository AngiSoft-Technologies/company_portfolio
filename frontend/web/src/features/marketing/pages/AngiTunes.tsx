...
export default function AngiTunes() {
  const product = useAsync(() => fetchProductBySlug('angitunes'), []);
        ...
        visual={
          <div className="relative h-72 w-full overflow-hidden rounded-3xl border border-[var(--border)] bg-[var(--surface)]">
            {p.image ? (
              ...
              <div className="flex h-full w-full items-center justify-center text-7xl text-[#0875FF]">
                {p.icon ? p.icon : '♪'}
              </div>
            )}
          </div>
