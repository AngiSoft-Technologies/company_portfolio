...
const NewsletterUnsubscribe = () => {
    ...
    useEffect(() => {
        ...
        apiGet(`/api/newsletter/unsubscribe?token=${encodeURIComponent(token)}`)
            .then((data) => {
                setStatus('success');
                setMessage(data?.message || 'You have been successfully unsubscribed. You will no longer receive updates.');
            })
            .catch((err) => {
                setStatus('error');
                setMessage(err?.response?.data?.error || err?.message || 'Failed to unsubscribe. The link may be invalid.');
            });
    }, [token]);
            ...
            >
                {status === 'loading' && (
                    <>
                        ...
                        <p className="mt-2" style={{ color: isDark ? 'rgba(255,255,255,0.5)' : 'rgba(0,0,0,0.5)' }}>
                            Removing you from the mailing list
                        </p>
                    </>
                )}
                ...
                {status === 'success' && (
                    <>
                        ...
                        <p className="mt-2 text-sm" style={{ color: isDark ? 'rgba(255,255,255,0.4)' : 'rgba(0,0,0,0.4)' }}>
                            We're sorry to see you go. You can always re-subscribe via our website.
                        </p>
                    </>
                )}
