...
export const themeSettings = (mode) => {
  ...
  return {
    ...
    typography: {
      fontFamily: ["Source Sans Pro", "Poppins", "sans-serif"].join(","),
      fontSize: 12,
...
export const useMode = () => {
  // Check localStorage for saved theme, fallback to system preference, then default to "dark"
  ...
  // Sync CSS variables, classes, and save to localStorage
  useEffect(() => {
    const root = document.documentElement;
    if (mode === "dark") {
      root.style.setProperty("--primary", "#141b2d");
      root.classList.add("dark");
      root.classList.remove("light");
    } else {
      root.style.setProperty("--primary", "#f2f0f0");
